...
export function utils_buildStructure(node: HTMLElement, id?: string) {
  let minX = 1000;
  let maxX = 0;
  let minY = 1000;
  let maxY = 0;
  ...
  function _walkDom(node: HTMLElement, structure: Structure, parent: string) {
    let _id = parent;
  ...
  function _getElProp(el: HTMLElement) {
    if (!el) {
      return false;
    }
